...
export const BlockOptionsSchema = z.object({
	name: z.string()
		...
		.regex(/^[a-z0-9-]+\/[a-z0-9-]+$/, 'Must be namespace/block-name (e.g. my-plugin/my-block)').default('namespace/yourblock'),
	title: z.string().min(1, 'Title is required').max(100).default('YourBlockName'),
	description: z.string().max(400).optional().default('Custom block generated with WP Block Composer'),
	category: z.enum(['text', 'media', 'design', 'widgets', 'theme', 'embed']).default('text'),
	icon: z.string().optional().default('block-default'),
	keywords: z.array(z.string()).max(3, 'WordPress recommends a maximum of 3 keywords').optional().default(['block', 'custom']),
	textdomain: z.string().optional().default('textdomain'),
	...
	attributes: z.record(
		z.object({
			type: z.string(),
			default: z.union([z.string(), z.array(z.any())]).nullable().default(null),
		})
	).optional().default({}),
})

export type BlockOptionsSchema = z.output<typeof BlockOptionsSchema>
export type BlockSupportsType = z.infer<typeof BlockOptionsSchema['shape']['supports']>
