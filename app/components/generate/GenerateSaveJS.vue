...
const { generatedSaveJs, saveJsHash } = storeToRefs(blockFileStore)
...
const currentInputHash = computed(() =>
  JSON.stringify({ fileType: 'save', blockType: blockType.value, blockConfig: blockConfigState.value, components: items.value })
)

const isStale = computed(() =>
  saveJsHash.value !== currentInputHash.value
)

const handleGenerate = async () => {
	console.log(saveJsHash.value);
...
<template>
  <GenerateFileLayout>
    ...
    <template #left-content>
      <div v-if="items.length" class="rounded-lg border border-monokai-blue/20 divide-y divide-monokai-blue/20">
      ...
      <div v-else class="rounded-lg border border-dashed border-monokai-accented p-6 text-center text-sm text-monokai-gutter ">
    ...
    <template #preview-content>
      ...
      <div v-else-if="isLoading" class="rounded-lg border border-monokai-blue/20 bg-monokai-blue/10 p-10 flex items-center justify-center">
        <UIcon name="i-lucide-loader-circle" class="animate-spin text-monokai-gutter size-6" />
      </div>
      <div v-else class="rounded-lg border border-dashed border-monokai-accented p-10 flex items-center justify-center text-sm text-monokai-gutter">
