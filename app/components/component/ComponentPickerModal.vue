...
function save() {
	...
	if (noOptions && notInnerBlocks) {
		nextTick(() => {
			attributeValueRef.value?.inputRef?.focus?.();
			attributeValueRef.value?.inputRef?.classList.add('ring-monokai-red!');
		})
		return
	}
...
<template>
	<UModal
		...
		:title="modalTitle"
		:ui="{
			content: 'sm:w-[800px] max-w-full!',
			overlay: 'bg-monokai-bg/10 backdrop-blur-sm'
		}"
	...
	>
		<template #body>
			...
			>
				...
				<div class="max-h-72 overflow-y-auto space-y-1">
					<button
						...
						:key="entry.name"
						class="w-full flex items-center gap-3 p-2.5 rounded-lg hover:bg-monokai-blue/10 text-left transition-colors"
					...
					>
						...
						<div class="min-w-0 flex-1">
							...
							<div class="text-xs text-monokai-gutter  truncate">
								{{ entry.description }}
							</div>
						</div>
						<div class="flex flex-col items-end gap-1 shrink-0">
							<UBadge
								...
								size="sm"
							/>
							<span class="text-[11px] text-monokai-gutter">{{ entry.registryPackage.replace('@wordpress/', '@wp/') }}</span>
						</div>
					</button>

					<p
						v-if="filteredEntries.length === 0"
						class="text-center text-sm text-monokai-gutter py-6"
					>
						No components found.
					</p>
				</div>
			...
			>
				<!-- Component info -->
				<div class="flex items-center gap-2 pb-2 border-b border-monokai-blue/20">
					<span
						class="w-3 h-3 rounded-full shrink-0"
						:style="{ backgroundColor: selectedEntry.color }"
					/>
					<span class="font-medium text-sm">{{ selectedEntry.name }}</span>
					<span class="text-xs text-monokai-gutter">{{ selectedEntry.registryPackage }}</span>
				...
				<!-- Description -->
				<p class="text-md text-monokai-yellow">
				...
				<template v-if="selectedEntry.options.length > 0 && selectedEntry.name !== 'InnerBlocks'">
					<UFormField
						...
						help="The block.json attribute name whose value this component reads from or writes to (e.g. 'content', 'url'). This is the field that maps to the onChange/value props to your block's attributes, enabling data persistence in the Block Editor."
					>
						<UInput
				...
				<p
					v-else
					class="text-sm text-monokai-gutter"
				>
