...
const modelPrompt = (
  ...
  - IMPORTANT: if MediaUpload is used, MediaUploadCheck must also be used.
	- CRITICAL: MediaUpload and MediaUploadCheck MUST both be imported from @wordpress/block-editor, NOT @wordpress/components. Importing either from @wordpress/components will break the block.
	- CRITICAL: When rendering the MediaUpload render prop (the open callback), ALWAYS use the WordPress <Button> component from @wordpress/components, NEVER a native <button> HTML element. Import Button from @wordpress/components. Example:
	  <MediaUploadCheck>
	    <MediaUpload
	      ...
	      render={({ open }) => (
	        <Button variant="primary" onClick={open}>
	          {url ? __('Replace Image') : __('Upload Image')}
	        </Button>
	      )}
	    />
	  </MediaUploadCheck>
	- CRITICAL: MediaUpload in edit.js MUST always render the selected media inline in the editor so the user can see what they uploaded. Render the image (or video/audio) directly in the edit area outside of InspectorControls, alongside the MediaUploadCheck/MediaUpload trigger. Example pattern:
<div className="image-grid">
          {images.length > 0 &&
            images.map((item) => (
              <div className="image-entry" key={item.id}>
                  <img src={item.url} />
              </div>
            ))
          }
	  ...
	  <MediaUploadCheck>
	    <MediaUpload
	      onSelect={(media) => setAttributes({ url: media.url, id: media.id, alt: media.alt })}
	      allowedTypes={['image']}
	      value={id}
	      render={({ open }) => (
	        <Button variant="primary" onClick={open}>
	  ...
	  For video/audio allowedTypes, render <video src={url} controls /> or <audio src={url} controls /> respectively instead of <img />.
	...
	- If the component tree includes InspectorControls but does NOT include a PanelBody, automatically wrap the InspectorControls children inside a <PanelBody title="[BlockTitle] Options"> where [BlockTitle] is the block's title from blockConfig (e.g. if title is "My Custom Block", use title="My Custom Block Options"). Import PanelBody from @wordpress/components alongside InspectorControls.
...
The following components store data that MUST appear in the rendered PHP output. All other components (InspectorControls, BlockControls, toolbar components, PanelBody, etc.) are editor-only and must be omitted from render.php entirely:
- \`RichText\`: \`<?= wp_kses_post($attributes['myAttr'] ?? ''); ?>\` inside the correct HTML tag — never drop RichText attributes.
- \`InnerBlocks\`: output \`<?= $content; ?>\` in the correct location in the DOM — required whenever InnerBlocks appears in the component tree.
- \`MediaUpload\` / \`MediaPlaceholder\`: render the stored media as the appropriate HTML tag. For images: \`<img src="<?= esc_url($attributes['url'] ?? ''); ?>" alt="<?= esc_attr($attributes['alt'] ?? ''); ?>" />\`. For video/audio allowedTypes, use \`<video>\` or \`<audio>\` with \`src="<?= esc_url($attributes['url'] ?? ''); ?>"\`.
- \`TextControl\` / \`TextareaControl\`: \`<?= esc_html($attributes['myAttrKey'] ?? ''); ?>\` inside the correct HTML element — never omit it.
- \`DatePicker\` / \`DateTimePicker\` / \`TimePicker\`: \`<time datetime="<?= esc_attr($attributes['myDate'] ?? ''); ?>"><?= esc_html($attributes['myDate'] ?? ''); ?></time>\`.
...
## Component rendering rules
The following components store data that MUST appear in the saved markup. All other components (InspectorControls, BlockControls, toolbar components, PanelBody, etc.) are editor-only and must be omitted from save.js entirely:
- \`RichText\`: use \`<RichText.Content tagName="..." value={attributes.myAttr} />\` — never drop RichText attributes.
- \`InnerBlocks\`: use \`<InnerBlocks.Content />\` — required whenever InnerBlocks appears in the component tree.
- \`MediaUpload\` / \`MediaPlaceholder\`: render the stored media as the appropriate HTML tag. For images: \`<img src={attributes.url} alt={attributes.alt} data-id={attributes.id} />\`. For video/audio allowedTypes, use \`<video>\` or \`<audio>\` with \`src={attributes.url}\`.
- \`TextControl\` / \`TextareaControl\`: render the stored string attribute inside the appropriate HTML element (p, span, div, h1–h6, etc.) based on context. Never omit it.
- \`DatePicker\` / \`DateTimePicker\` / \`TimePicker\`: render as \`<time dateTime={attributes.myDate}>{attributes.myDate}</time>\`.
